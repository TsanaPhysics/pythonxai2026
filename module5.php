...
<header class="page-header">
    <div class="container">
        <h1>บทที่ 5: Digital Twin Simulation</h1>
        <p>จำลองโลกเกษตรเสมือนจริง เชื่อมโยงโค้ด AI เข้ากับภาพเคลื่อนไหว</p>
    </div>
</header>
...
<main class="container">
    <div style="display: grid; grid-template-columns: 1fr 1fr; gap: 40px; align-items: start; margin-bottom: 60px;">
        <div class="card">
            <div class="card-img" style="height: 400px;"><img src="assets/images/holo_workshop.png" alt="Module Image"></div>
        </div>
        <div>
            <h2 style="color: var(--secondary); margin-bottom: 20px;">เรียนรู้นวัตกรรมบทที่ 5</h2>
            ...
            <div style="margin-top: 30px;">
                <a href="#code-gallery" class="btn" style="width:100%;">เลื่อนไปดูรหัสคำสั่ง 5 ทั้งหมด</a>
            </div>
        </div>
    </div>
    ...
    <div id="code-gallery" style="margin-top: 80px;">
        ...
        <div class="accordion">
            <?php
            $ch_padded = str_pad(5, 2, '0', STR_PAD_LEFT);
        ...
        <!-- Minecraft Bonus Section -->
        <h3 style="color: #2e7d32; margin: 40px 0 20px; display: flex; align-items: center; gap: 10px;">
            <img src="assets/images/research.png" style="width: 30px;"> ⛏️ Minecraft Bonus Section
        </h3>
        <div class="accordion">
            <?php
            $mc_dir = "workshop_code/python_scripts/ch05_pygame_zero/minecraft_bonus";
            if(is_dir($mc_dir)){{
                $rel_dir = "python_scripts/ch05_pygame_zero/minecraft_bonus";
                $fs = array_diff(scandir($mc_dir), array('.', '..', '.DS_Store'));
                $idx = 1;
                foreach($fs as $f){{
                    $content = file_get_contents($mc_dir."/".$f);
                    $id_tag = "mc-bonus-" . $idx;
                    ?>
                    <div class="accordion-item" style="border-left: 5px solid #2e7d32;">
                        <div class="accordion-header" onclick="toggleAcc('<?php echo $id_tag; ?>')">
                            <h4 style="color: #2e7d32;">⛏️ <?php echo $f; ?> (Minecraft Simulation)</h4>
                            <span style="color: #999;">Click to Preview</span>
                        </div>
                        <div id="<?php echo $id_tag; ?>" class="accordion-content">
                            <div class="code-viewer-container">
                                <div class="code-viewer-controls" style="background: #1b5e20;">
                                    <button class="btn-sm btn-copy" onclick="copyToClipboard('code-<?php echo $id_tag; ?>')">Copy โค้ด</button>
                                    <a href="workshop_code/<?php echo $rel_dir; ?>/<?php echo $f; ?>" download class="btn-sm" style="background: #ffd54f; color: #333; text-decoration:none;">Download</a>
                                </div>
                                <pre class="language-python" style="margin: 0; border-radius: 0;"><code id="code-<?php echo $id_tag; ?>"><?php echo htmlspecialchars($content); ?></code></pre>
                            </div>
                        </div>
                        ...
                        <div id="<?php echo $id_tag; ?>" class="accordion-content">
                            <div style="padding: 15px 30px; background: #fdfdfd; border-bottom: 1px solid #eee; display: flex; justify-content: space-between; align-items: center;">
                                <div style="display: flex; gap: 10px; align-items: center;">
                                    ...
                                    <a href="https://colab.research.google.com/github/TsanaPhysics/pythonxai2026/blob/main/<?php echo $dir; ?>/<?php echo $f; ?>" target="_blank" class="btn-sm" style="background: #f39c12; color: white; text-decoration: none; padding: 5px 12px; border-radius: 4px; font-size: 0.8rem;">Google Colab</a>
                                    <a href="https://mybinder.org/v2/gh/TsanaPhysics/pythonxai2026/main?labpath=<?php echo $dir; ?>/<?php echo $f; ?>" target="_blank" class="btn-sm" style="background: #2ecc71; color: white; text-decoration: none; padding: 5px 12px; border-radius: 4px; font-size: 0.8rem;">JupyterLab</a>
                                </div>
    ...
    <div style="margin-top: 60px; padding: 40px; background: var(--bg-light); border: 2px dashed var(--primary); border-radius: 20px;">
        ...
        <div style="display: flex; gap: 20px;">
            ...
            <a href="module6.php" class="btn">ไปที่บทเรียนถัดไป &raquo;</a>
        </div>
    </div>
</main>
...
<script>
    ...
    document.querySelectorAll('.nav-links a').forEach(a => {
        if(a.innerText.includes('บทที่ 5')) a.classList.add('active');
    });
</script>
